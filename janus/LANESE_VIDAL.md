...
| "procedures have neither parameters nor local variables" (text L121–124) | (as of 2026-09-24) **larger**: parameters (`Γ : pid -> proc`, by reference) and `local`/`delocal` blocks (`RULES.md` §1) | Ours contains theirs in these two respects. Arrays: theirs has `x[e1] ⊕= e2` (Fig. 1, text L106–107); ours has `a[e1] op= e2` since 2026-09-25 (`RULES.md` §1), so our fragment now contains theirs, except that we omit `/` and `%` and make out-of-bounds reads total. |
...
- **The control-token semantics for the Janus core itself** — conditionals, `do`-part loops,
  ...
  parameters, no locals), **arrays included** (2026-09-25), and beyond it with call-by-reference **parameters**
  ...
  carries over. Whether it "scales" in their sense is answered for their whole fragment
  except `/` and `%` (RULES.md §7).
- **Per-rule partial injectivity, mechanized.** The property proved is that each of the 31
...
| Relation to big-step | `equiv` (L320) | `exec_iff_pc` | `exec_iff_jstar`; `exec_inv_iff` (inverter correct for big-step) is **derived** from `inv_step_reverses`, whereas PyJanus proves `exec_iff` directly on the big-step side |
...
| Big-step inverter correctness | Known: Theorem 4 (Statement inverter) of Yokoyama–Glück, PEPM 2007 (`σ ⊢ s ⇒ σ′ ⟺ σ′ ⊢ I⟦s⟧ ⇒ σ`; checked in the paper itself 2026-09-26; LLS L149 cites it as [20, Theorem 4], and an earlier version of this row misattributed it to Yokoyama–Axelsen–Glück 2008, whose Theorem 1 is forward/backward determinism); mechanized by PPR (Matita) and PyJanus (`exec_iff`). LV's Lemma 4 is the static `flow⁻¹(s) = flow(I⟦s⟧)`. | **not new as a statement**; new only as a *derivation* from step-level reversibility (`exec_inv` via `jstar_inv`) |
