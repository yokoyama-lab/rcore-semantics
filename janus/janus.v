(*
  ...
  carried over to a language with
      ...
      call p / uncall p   against a procedure environment Gamma,
  ...
  Scope.  This is Janus WITHOUT arrays, without local/delocal, without
  procedure parameters, and without the division-like operators / and %
  (whose partiality is orthogonal to the control-token question).  Values
...
Inductive cont_stmt : Type :=
  ...
  | CS_loop_loop  (e1 : expr) (s1 : stmt) (cs : cont_stmt) (e2 : expr)
      (* token inside s2 *)
  | CS_call       (p : pid) (cs : cont_stmt)   (* token inside the body Gamma p *)
  | CS_uncall     (p : pid) (cs : cont_stmt).  (* token inside inv (Gamma p) *)
...
Inductive jstep (Γ : penv) : cont_stmt * store -> cont_stmt * store -> Prop :=
  ...
  | J_Call_Enter : forall p s,
      jstep Γ (CS_pre (Scall p), s) (CS_call p (CS_pre (Γ p)), s)
  | J_Call_Exit : forall p s,
      jstep Γ (CS_call p (CS_post (Γ p)), s) (CS_post (Scall p), s)
  | J_Uncall_Enter : forall p s,
      jstep Γ (CS_pre (Suncall p), s) (CS_uncall p (CS_pre (inv (Γ p))), s)
  | J_Uncall_Exit : forall p s,
      jstep Γ (CS_uncall p (CS_post (inv (Γ p))), s) (CS_post (Suncall p), s)
  ...
  | J_Ctx_Call : forall p cs cs' s s',
      jstep Γ (cs, s) (cs', s') ->
      jstep Γ (CS_call p cs, s) (CS_call p cs', s')
  | J_Ctx_Uncall : forall p cs cs' s s',
      jstep Γ (cs, s) (cs', s') ->
      jstep Γ (CS_uncall p cs, s) (CS_uncall p cs', s').
...
Inductive wf_stmt : stmt -> Prop :=
  ...
  | wf_Scall   : forall p, wf_stmt (Scall p)
  | wf_Suncall : forall p, wf_stmt (Suncall p).

Definition wf_penv (Γ : penv) : Prop := forall p, wf_stmt (Γ p).
...
Inductive wf_cs : cont_stmt -> Prop :=
  ...
  | wf_cs_call       : forall p cs, wf_cs cs -> wf_cs (CS_call p cs)
  | wf_cs_uncall     : forall p cs, wf_cs cs -> wf_cs (CS_uncall p cs).
...
Proof.
  destruct st as [ | x op e | a b | e1 a b e2 | e1 a b e2 | p | p].
  - left; constructor.
...
Ltac wf_inv :=
  ...
  | [ H : wf_cs (CS_call _ _)            |- _ ] => inversion H; subst; clear H
  | [ H : wf_cs (CS_uncall _ _)          |- _ ] => inversion H; subst; clear H
...
Fixpoint cs_inv (cs : cont_stmt) : cont_stmt :=
  ...
  | CS_call p cs1             => CS_uncall p (cs_inv cs1)
  | CS_uncall p cs1           => CS_call p (cs_inv cs1)
  end.
...
Proof.
  ...
  - apply J_Uncall_Exit.
  - apply J_Uncall_Enter.
  - rewrite inv_involutive. apply J_Call_Exit.
  - rewrite inv_involutive. apply J_Call_Enter.
...
Fixpoint step_fun (Γ : penv) (cs : cont_stmt) (s : store)
  ...
  | CS_pre (Scall p) => Some (CS_call p (CS_pre (Γ p)), s)
  | CS_pre (Suncall p) => Some (CS_uncall p (CS_pre (inv (Γ p))), s)
  | CS_post _ => None
  | CS_seq_L cs1 s2 =>
  ...
  | CS_call p cs1 =>
      match step_fun Γ cs1 s with
      | Some (c, t) => Some (CS_call p c, t)
      | None => match cs1 with
                | CS_post b => if stmt_eq_dec b (Γ p)
                               then Some (CS_post (Scall p), s) else None
                | _ => None
                end
      end
  | CS_uncall p cs1 =>
      match step_fun Γ cs1 s with
      | Some (c, t) => Some (CS_uncall p c, t)
      | None => match cs1 with
                | CS_post b => if stmt_eq_dec b (inv (Γ p))
                               then Some (CS_post (Suncall p), s) else None
                | _ => None
                end
      end
  end.
...
Proof.
  ...
  - (* CS_call *)
    ...
    + destruct cs; try discriminate.
      destruct (stmt_eq_dec st (Γ p)) as [-> | Hne]; [| discriminate].
      injection H as ? ?; subst; constructor.
  - (* CS_uncall *)
    destruct (step_fun Γ cs s) as [[c t]|] eqn:E.
    + injection H as ? ?; subst. apply J_Ctx_Uncall. apply IHcs; exact E.
    + destruct cs; try discriminate.
      destruct (stmt_eq_dec st (inv (Γ p))) as [-> | Hne]; [| discriminate].
      injection H as ? ?; subst; constructor.
Qed.
...
Definition Γ0 : penv := fun _ => Sskip.
