...
sequence, `if–fi`, `from–do–loop–until`, `call`/`uncall` against a procedure environment —
extending the R-CORE semantics of `../proofs.v` to the fragment of Lanese–Vidal (RC 2026)
without arrays, `local`/`delocal`, procedure parameters, `/` and `%`. Self-contained: it does
...
- `janus.v` — syntax, `jstep Γ` (27 rules), `bstep`, `inv`/`cs_inv`, well-formedness with decision procedures, `step_fun`, all proofs (36 results, axiom-free, Rocq 9.1.1).
...
Excluded for now: arrays, `local`/`delocal`, procedure parameters, `/` and `%`.
...
2. `local x = e … delocal x = e`: a second data-carrying rule pair; delocal is a value assertion.
